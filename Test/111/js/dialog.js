...
Dialog = function (a) {
    ...
    this.style = function () {
        ...
        for (let i in l) if (l[i].rel === "stylesheet" && e.href === l[i].href) e = l[i];
        document.body.appendChild(e);
        return e;
    };
    // Определение формата картинки
    this.type = function () {
    ...
    // Само открытие картинки и создание окна
    ...
    // Создание самого крестика и кнопки перетаскивания
    this.id = a.id || "dialog";
    ...
    for (let i in p) d[p[i]] = a[p[i]] !== undefined ? a[p[i]] : undefined;
...
// Функция создания елемента
Dialog.add = function (n) {
    var e = document.createElement(n);
    document.body.appendChild(e);
    return e;
};
// Функция создания тега script
Dialog.inc = function (a) {
    var s = document.createElement("script");
    s.src = a;
    document.getElementsByTagName("head")[0].appendChild(s);
};
// Функция создания пути до скрипта
Dialog.path = function () {
    var p, r = "", s = document.getElementsByTagName('script');
    for (let i in s) if (s[i].src && s[i].src.indexOf('dialog.js') > 0) p = s[i].src.split("/").slice(0, -1);
    var d = location.href.split("/").slice(0, -1);
    for (let i in d) if (d[i] !== p[i]) r += "../";
    for (let i in p) if (d[i] !== p[i]) r += p[i] + "/";
    return r;
}();
// Функция нахождения путей для перетаскивания и самой картинки
...
Dialog.objects = [];
// Функция создания объекта Dialog
function dialog(url, a, fn) {
    ...
    var d = new Dialog(a);
    d.html ? f_get(Dialog.path + d.html, function (t) {
        d.tpl = t;
        d.open.call(d);
    }) : d.open();
}